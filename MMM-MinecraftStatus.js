...
Module.register("MMM-MinecraftStatus", {
  ...
  getDom: function () {
    ...
    if (this.config.alertSoundWhenPingFails) {
      this.audioPlayerPingFailed = this.createAudioPlayer(
        topDiv,
        this.config.alertSoundWhenPingFails
      );
    }

    if (this.config.alertSoundOnFirstPlayer) {
      this.audioPlayerFirstPlayer = this.createAudioPlayer(
        topDiv,
        this.config.alertSoundOnFirstPlayer
      );
    }
    ...
    this.playersDiv = this.createMetricPanel(
      this.successTbody,
      "user",
  ...
  triggerHelper: function () {
    ...
    this.sendSocketNotification("MINECRAFT_SERVER_PLAYER_LIST", {
      identifier: this.identifier, // sent to helper and sent back to us below
      hostname: this.config.hostname,
      port: this.config.port,
    });
  ...
  startTimer: function () {
    ...
    if (!this.timer) {
      this.timer = setInterval(
        function () {
          self.triggerHelper();
        }, // "self" is "this" for MMM-MinecraftStatus object
    ...
    if (payload.identifier === this.identifier) {
      switch (notification) {
        case "MINECRAFT_UPDATE":
          ...
          if (
            this.audioPlayerFirstPlayer &&
            this.lastPingNumPlayers == 0 &&
            currPlayers > 0
          ) {
        ...
        case "MINECRAFT_ERROR":
          ...
          this.sendNotification("SHOW_ALERT", {
            // picked-up by default module "alert"
            type: "notification",
